...
$totalCount = $m_users_follow->count('uid ='.$uid);
$pageurl = U('friend','followers',array('page'=>'%page%','id'=>$uid));
...
$rows = $m_users_follow->findAll(array(
    'field'=>'u.*',
    'table' => '#users_follow as f inner join #users as u on f.follow_uid = u.id',
    'where' => 'f.uid='.$uid,
...
if(isAjax()){
    ...
    $site_title = '粉丝列表 - '.getSetting('site_title');
    $view->assign('site_title',$site_title);
    $view->assign('totalCount',$totalCount);
    $view->display('friend/followers.php');
}