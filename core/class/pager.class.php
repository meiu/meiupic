...
class Pager
{
    ...
    public function __construct($curPage = 1, $pageSize = 20, $totalItems, $url = '')
    {
        ...
        $this->url          = str_replace('%25page%25', '%page%', $url);
    ...
    /**
     * 获取分页的数据库offset，和其他信息
     * @return array
     */
    public function getLimit(){        
        if($this->totalItems>0){
            $arr['start'] = ($this->curPage-1)*$this->pageSize;
        ...
        $arr["total"]= $this->totalPages;//总数页数
        $arr['current']=$this->curPage; //当前页
        $arr['count']=$this->totalItems;
        return $arr;
    }
    /**
     * Prefix HTML
